package com.bridgelabz.UC9viewpersonbycity;
...
public class AddressBook {
	...
	public void addPerson() {
		...
		if (isDuplicate) {
			...
			if (personsByCity.get(contacts.getCity()) == null)
				personsByCity.put(contacts.getCity(), new ArrayList<>());
			personsByCity.get(contacts.getCity()).add(contacts);
			if (personsByState.get(contacts.getState()) == null)
				personsByState.put(contacts.getState(), new ArrayList<>());
	...
	public void searchByCity(String city, String firstName) {
		Predicate<Contacts> searchPerson = (contact -> contact.getCity().equals(city)
	...
	public void searchByState(String state, String firstName) {
		Predicate<Contacts> searchPerson = (contact -> contact.getState().equals(state)
				&& contact.getFirstName().equals(firstName));
		referenceBook.stream().filter(searchPerson).forEach(contacts -> output(contacts));
	}
	...
	public void editContact(String name) {
		...
		for (i = 0; i < referenceBook.size(); i++) {
			if (name.equals(referenceBook.get(i).getFirstName()))
				break;
		}
